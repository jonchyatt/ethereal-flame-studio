...
must_haves:
  truths:
    - "User receives gentle nudges before upcoming deadlines without being interrupted mid-task"
    - "User can acknowledge nudges by tapping OR speaking, whichever is convenient"
    - "User hears midday progress summary when scheduled"
    - "User can skip check-ins with a single word or tap"
    - "User is prompted to capture loose ends at end of work day"
  artifacts:
