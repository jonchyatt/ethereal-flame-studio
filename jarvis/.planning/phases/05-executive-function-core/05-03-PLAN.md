...
must_haves:
  truths:
    ...
    - "Dashboard updates after user completes a task via voice command"
  artifacts:
  ...
  key_links:
    ...
    - from: "toolExecutor.ts"
      to: "dashboardStore"
      via: "refresh trigger after writes"
      pattern: "triggerDashboardRefresh|refreshDashboard"
---
...
```
  ...
  <done>Dashboard store manages section visibility and refresh trigger, PriorityIndicator shows attention dots</done>
</task>
...
<task type="auto">
  <name>Task 3: Create DashboardPanel, wire refresh on Notion writes, and integrate into page</name>
  <files>
    - src/components/jarvis/Dashboard/DashboardPanel.tsx
...
**Update `src/lib/jarvis/notion/toolExecutor.ts` to trigger dashboard refresh after writes:**

Find the section where write tools are executed (create_task, update_task_status, mark_bill_paid, pause_task, add_project_item) and add refresh trigger after successful writes:

```typescript
import { useDashboardStore } from '@/lib/jarvis/stores/dashboardStore';

// After successful write operations (create_task, update_task_status, etc.):
// This triggers dashboard to refetch data
...
// In each write tool handler, after successful API call:
// e.g., in update_task_status handler:
// await notionClient.updatePage(...)
// triggerDashboardRefresh();
```

Write tools that should trigger refresh (all from Phase 4):
- `create_task` - New task appears
- `update_task_status` - Task completion updates count
- `mark_bill_paid` - Bill removed from due list
- `pause_task` - Task visibility changes
- `add_project_item` - May affect task list
...
2. Add dashboard data state and fetching with refresh support:
```typescript
...
// Also refresh periodically (every 5 minutes) as backup
useEffect(() => {
...
9. Complete a task via voice command -> dashboard updates automatically
  </verify>
  <done>Dashboard panel renders alongside orb with responsive layout, refreshes after Notion writes</done>
</task>
...
10. Data refreshes every 5 minutes (polling)
11. Data refreshes immediately after voice command completes a task
</verification>
...
- Dashboard refreshes immediately when user completes task via voice (not just polling)
</success_criteria>
