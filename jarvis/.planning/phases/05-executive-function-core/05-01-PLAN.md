...
2. Call these existing tools in parallel:
...
3. **Parse tool results** into BriefingData structure:
...
4. **buildCheckInData(type: 'midday' | 'evening')** - for check-ins (used in Plan 02)

5. **buildStreakSummary(habits)** - Creates speech-friendly habit summary:
...
```
  ...
  <done>BriefingBuilder aggregates Notion data, BriefingFlow delivers section-by-section walkthrough</done>
</task>
...
- BriefingBuilder fetches all data in parallel from Notion
