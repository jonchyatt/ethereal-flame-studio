...
3. VoicePipelineState type exported: `grep "export.*VoicePipelineState" src/lib/jarvis/types.ts`
  </verify>
...
export class VoicePipeline {
  ...
  private ttsClient: SpeechClient;
  ...
  constructor(config: VoicePipelineConfig = {}) {
    ...
    this.ttsClient = new SpeechClient({
      onStart: () => {
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
Complete voice pipeline: user speaks via PTT, speech is transcribed by Deepgram, echo response is synthesized by browser SpeechSynthesis, orb animates through all states.
  </what-built>
  <how-to-verify>
**Prerequisites:**
- DEEPGRAM_API_KEY in .env.local
- (No TTS API key needed - uses browser SpeechSynthesis)
