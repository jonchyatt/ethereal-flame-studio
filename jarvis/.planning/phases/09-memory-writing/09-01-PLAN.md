...
<task type="auto">
  <name>Task 2: Run database migration for deletedAt column</name>
...
```bash
# For SQLite/Turso, verify table structure
sqlite3 [db-path] "PRAGMA table_info(memory_entries);" 2>/dev/null || \
...
# Then check http://localhost:4983 shows deletedAt column
```
  </verify>
  <done>Database has deletedAt column in memory_entries table</done>
</task>

<task type="auto">
  <name>Task 3: Add soft delete and restore query functions</name>
...
<task type="auto">
  <name>Task 4: Add fuzzy matching function for forget requests</name>
...
2. Database migration applied (deletedAt column exists in DB)
3. softDeleteMemoryEntry sets deletedAt timestamp
4. restoreMemoryEntry clears deletedAt
5. getDeletedMemories returns recently deleted entries
6. getMemoryEntries excludes deleted by default
7. findMemoriesMatching returns scored matches
8. MemoryService.forget(), .restore(), .findForForget() work
