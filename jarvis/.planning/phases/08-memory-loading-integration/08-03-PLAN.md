...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/api/jarvis/chat/route.ts"
      ...
      via: "buildSystemPrompt call with proactiveSurfacing"
      pattern: "buildSystemPrompt.*proactiveSurfacing"
---
...
<task type="auto">
  <name>Task 3: Integrate proactive surfacing into chat API via buildSystemPrompt</name>
  <files>src/app/api/jarvis/chat/route.ts</files>
  <action>
Modify `src/app/api/jarvis/chat/route.ts` to include proactive surfacing via `buildSystemPrompt()`:
...
3. Pass proactive surfacing to `buildSystemPrompt()`:
```typescript
...
Key points:
- Consistent with 08-02: All prompt building done via `buildSystemPrompt()`, no string concatenation
- `proactiveSurfacing` passed as parameter, handled inside `buildSystemPrompt()`
- Empty surfacing converted to undefined to avoid adding empty sections to prompt
- When flag is off, both `memoryContext` and `proactiveSurfacing` are undefined, v1 behavior preserved
  </action>
  <verify>
TypeScript compiles:
```bash
...
```
  ...
  <done>
Chat API generates proactive surfacing guidance and passes it to `buildSystemPrompt()` alongside memory context. Consistent with 08-02 approach.
  </done>
</task>
...
- All prompt building via `buildSystemPrompt()` (consistent with 08-02)
