/**
 * Ingest pipeline — inlined here so Vercel can bundle it without needing
 * the /worker/ directory (which is excluded via .vercelignore).
 *
 * Original source: worker/pipelines/ingest.ts
 */
 ...
 *   - `youtube`    — download via yt-dlp (falls back to @distube/ytdl-core)
...
export async function runIngestPipeline(
  ...
  try {
      ...
      case 'url': {
        ...
        while (true) {
          ...
          if (downloaded > MAX_FILE_SIZE_BYTES) {
            throw new Error(`Download exceeds ${config.maxFileSizeMB}MB limit`);
          }
    ...
    // Final file size check (post-extraction)
