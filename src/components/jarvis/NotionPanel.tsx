...
export function NotionPanel() {
  const { isOpen, currentUrl, currentLabel, currentCluster, mode, closePanel, history, goBack, teachContent } =
    useNotionPanelStore();
          ...
          {mode === 'teach' && teachContent ? (
            ...
            <div className="px-6 py-4">
              ...
              <p className="text-white/50 text-sm mb-6">
                Follow along as Jarvis guides you through each step
              </p>

              {/* Step list */}
              <div className="space-y-2 mb-6">
                {teachContent.steps.map((step, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 px-3 py-2 rounded-lg bg-white/5 border border-white/10"
                  >
                    <div className="w-6 h-6 rounded-full bg-cyan-500/20 border border-cyan-400/30
                      flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-cyan-400 text-xs font-medium">{idx + 1}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white/80 text-sm font-medium">{step.title}</p>
                      {step.panelNote && (
                        <p className="text-white/40 text-xs mt-1">{step.panelNote}</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-2 w-full px-6 py-3
                  bg-white/10 hover:bg-white/15 border border-white/20 rounded-xl
                  text-white text-sm font-medium transition-colors active:scale-95 mb-4"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
                Open in Notion to Practice
              </a>

              <p className="text-white/30 text-xs text-center">
                Say &quot;continue&quot; or &quot;got it&quot; to advance through steps
              </p>
            </div>
          ) : (
