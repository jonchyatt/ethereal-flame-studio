...
<task type="auto">
  ...
  <action>
1. In `src/app/api/webhooks/worker/route.ts`:
...
2. In `src/app/api/audio/jobs/[jobId]/route.ts` (the poll endpoint):
...
3. Structure of the enriched poll response for a completed render job:
   ```json
   {
     "jobId": "...",
     "status": "complete",
