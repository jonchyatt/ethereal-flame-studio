...
must_haves:
  truths:
    - "User submits a render via POST /api/render and receives a jobId immediately without blocking"
    - "User can poll the jobId and see the render job progressing through worker stages"
    - "Render job reaches Modal with audio accessible via a signed R2 URL (no local file dependency)"
  artifacts:
    ...
    - path: "src/lib/render/modalClient.ts"
      provides: "Updated Modal client accepting audioSignedUrl, webhookUrl, and webhookSecret"
  ...
  key_links:
    ...
    - from: "worker/pipelines/render.ts"
      to: "src/lib/render/modalClient.ts"
      via: "submitToModal with webhookUrl and webhookSecret"
      pattern: "webhookUrl|webhookSecret"
---
...
2. In `src/app/api/render/route.ts`, refactor the POST handler to dispatch through JobStore instead of directly calling Modal:
   ...
   - Remove the direct Modal dispatch code (the `if (process.env.MODAL_ENDPOINT_URL)` block) from POST -- the worker handles this now
   - Remove the BullMQ dispatch code from POST -- the worker handles this now
   - In POST only: remove the `ServerJobStore.create()` call and all ServerJobStore usage (no dual-write). The render job flows through JobStore only.
   - IMPORTANT: Keep the `import { ServerJobStore }` at the top of the file and keep the GET handler EXACTLY as-is. The GET handler uses `ServerJobStore.list()` and `ServerJobStore.getStatusCounts()` for the legacy render job list UI, and this must remain functional. Only POST logic changes.
...
1. Update `src/lib/render/modalClient.ts`:
   ...
   - Add `webhook_url?: string` and `webhook_secret?: string` to `ModalSubmitRequest` interface
   - In `submitToModal`, accept new optional parameters: `audioSignedUrl?: string`, `webhookUrl?: string`, `webhookSecret?: string`
   - When `audioSignedUrl` is provided, include it as `audio_signed_url` in the request body (this is the R2 presigned download URL)
   - When `webhookUrl` is provided, include it as `webhook_url` in the request body (the Modal Python wrapper will forward this as `--webhook-url` to modal-render-entry.ts)
   - When `webhookSecret` is provided, include it as `webhook_secret` in the request body (the Modal Python wrapper will forward this as `--webhook-secret` to modal-render-entry.ts)
...
2. Create `worker/pipelines/render.ts` following the existing pipeline pattern (see ingest.ts for reference):
   ...
   - Step 5 - Dispatch to Modal: Derive `webhookUrl` as `${process.env.NEXT_PUBLIC_APP_URL || 'https://' + process.env.VERCEL_URL}/api/webhooks/worker` and `webhookSecret` as `process.env.INTERNAL_WEBHOOK_SECRET`. Call `submitToModal({ config, jobId: job.jobId, audioSignedUrl: signedUrl, webhookUrl, webhookSecret })`. Store the `call_id` from the response in the job metadata via `store.update(job.jobId, { stage: 'dispatched', progress: 30, result: { modalCallId: response.call_id } })`.
