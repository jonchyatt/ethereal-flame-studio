...
files_modified:
  ...
  - public/presets/ethereal-flame.jpg
  - public/presets/ethereal-mist.jpg
  - public/presets/cosmic-void.jpg
  - public/presets/solar-flare.jpg
  - public/presets/aurora.jpg
  - public/presets/neon-pulse.jpg
autonomous: true

must_haves:
  truths:
    - "6 built-in presets exist with distinct visual styles"
    - "Each preset has curated settings that create recognizable aesthetic"
    - "Presets cover range from calm (mist) to energetic (flame, pulse)"
  artifacts:
    - path: "src/lib/templates/builtInPresets.ts"
      provides: "BUILT_IN_PRESETS array with 6 curated presets"
      exports: ["BUILT_IN_PRESETS", "initializeBuiltInPresets"]
  key_links:
    - from: "src/lib/templates/builtInPresets.ts"
      to: "src/lib/stores/visualStore.ts"
      via: "ETHEREAL_FLAME_CONFIG, ETHEREAL_MIST_CONFIG imports"
      pattern: "import.*ETHEREAL"
---

<objective>
Create 6 built-in curated presets with distinct visual styles.

Purpose: Provide users with a starting palette of professionally-tuned visual configurations they can use immediately or customize (TPL-03).

Output: Built-in presets file with 6 curated templates covering different aesthetics: Ethereal Flame, Ethereal Mist, Cosmic Void, Solar Flare, Aurora, Neon Pulse.
</objective>
...
<task type="auto">
  <name>Task 1: Create built-in presets definitions</name>
  <files>src/lib/templates/builtInPresets.ts</files>
  <action>
Create `src/lib/templates/builtInPresets.ts` with 6 curated presets:
...
The 6 presets provide a range of aesthetics:
1. **Ethereal Flame** - Classic warm fire (default)
2. **Ethereal Mist** - Soft and calm
3. **Cosmic Void** - Dark and minimal
4. **Solar Flare** - Explosive and reactive
5. **Aurora** - Cool and flowing
6. **Neon Pulse** - Vibrant synthwave
  </action>
  <verify>
1. TypeScript compiles: `npx tsc --noEmit`
2. BUILT_IN_PRESETS array has exactly 6 items
  </verify>
  <done>6 built-in presets defined with distinct visual styles covering calm to energetic range</done>
</task>

<task type="auto">
  <name>Task 2: Create placeholder thumbnail images</name>
  <files>public/presets/</files>
  <action>
Create the `public/presets/` directory and placeholder images.

Since we cannot generate actual screenshots until the UI is built, create simple placeholder gradient images using a data URL approach or note that thumbnails will be generated after plan 02-04.

For now, update the built-in presets to use a fallback:

1. Create directory: `public/presets/`
2. Add a comment in builtInPresets.ts noting:
   ```typescript
...
3. The TemplateCard component (plan 02-03) will handle missing thumbnails gracefully with:
   ```tsx
   {template.thumbnail ? (
...
Thumbnails will be captured during manual testing after all UI is built.
  </action>
  <verify>
Directory exists: `public/presets/`
  </verify>
  <done>Placeholder strategy for thumbnails documented, directory created, UI will handle missing thumbnails gracefully</done>
</task>
...
2. `BUILT_IN_PRESETS.length === 6`
3. Each preset has distinct layer configurations
4. `public/presets/` directory exists
</verification>

<success_criteria>
- 6 built-in presets exist with distinct visual styles
- Each preset has name, description, and curated settings
- Presets range from calm (Mist, Void) to energetic (Flare, Pulse)
- initializeBuiltInPresets function ready for app startup
</success_criteria>
