...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/api/audio/assets/[id]/stream/route.ts"
      to: "src/lib/storage/index.ts"
      via: "storage.get for file content"
      pattern: "storage\\.(get|getSignedUrl)"
---
...
**`src/app/api/audio/assets/[id]/stream/route.ts`:**
- Instead of `fs.readdir` + `createReadStream`, use `storage.get()` to read the full file content
- For Range request support: use `storage.get()` for the full buffer and slice it (acceptable for MVP; Phase 14 will optimize with proper streaming from R2)
- Alternatively, redirect to a signed URL: `return NextResponse.redirect(await storage.getSignedUrl(key))` -- **prefer this approach for production** since it offloads bandwidth to R2/CDN. For local dev, the signed URL points back to the local API route.
- Implementation: Check if `STORAGE_BACKEND === 'r2'` -- if so, redirect to signed URL. If local, serve the file directly from storage (preserving Range support).
...
- Stream route redirects to signed URL in production, serves directly in local dev
